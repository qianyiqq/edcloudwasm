...
const manualPipe = async (readable, writable, close) => {
    ...
    const flushBuffer = () => {
        offset > 0 && (offset > safeBufferSize ? (writable.send(buffer.subarray(0, offset)), buffer = new Uint8Array(bufferSize)) : writable.send(buffer.slice(0, offset)), offset = 0);
    ...
    try {
        while (true) {
            ...
            buffer.set(value, offset), offset += chunkLen;
